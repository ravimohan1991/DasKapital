...
It is clear that this is a game of contexts, meaning, we are bringing two commodities in a single context of use values.  So we will fix the physical dimesion\footnote{If you are unaware of dimensional analysis, please consult \href{https://en.wikipedia.org/wiki/Dimensional_analysis}{this}.} of use values to be $[\omega]$\footnote{I am so tempted to use `uv' initials here, but umm $\ldots$.}.  The naming convention of units, of dimension $\omega$, shall be
\begin{equation}
...
\mathbb{S}_{\text{linen}} =  \{\ldots,6yd, 4yd, yd, \ldots\},
\end{equation}
where, `$yd$' stands for yards, with each element having fixed dimension $\omega$ and the following usual conversion
\begin{equation}
  \label{eq:usevalconv}
  {6yd}_{\text{linen}} = 6\cdot {yd}_{\text{linen}}.
\end{equation}

If the use value is gauged without any physical measure, then we write `$nv$' for null-value.
